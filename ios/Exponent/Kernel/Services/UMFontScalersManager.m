...
- (UIFont *)UMFontWithSize:(CGFloat)fontSize
{
  for (id<UMFontScaler> fontScaler in currentFontScalers) {
  ...
  return [self UMFontWithSize:fontSize];
}
...
@implementation UMFontScalersManager
...
+ (void)initialize
{
  ...
  dispatch_once(&initializeCurrentFontScalersOnce, ^{
    ...
    SEL exUpdate = @selector(UMFontWithSize:);
...
- (void)registerFontScaler:(id<UMFontScaler>)fontScaler
{
